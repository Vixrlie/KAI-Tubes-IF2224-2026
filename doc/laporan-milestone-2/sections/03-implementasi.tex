...
Fungsi \texttt{parseProgram} menjadi entry point yang menyusun urutan \texttt{program-header}, \texttt{declaration-part}, \texttt{compound-statement}, dan token \texttt{period}. Setelah itu parser memastikan tidak ada token tersisa. Skema ini mengikuti aturan produksi utama pada grammar Arion dan memastikan program selalu ditutup dengan \texttt{period}.

\subsection{Pemilihan Produksi Dengan Lookahead}
Pemilihan produksi dilakukan dengan lookahead satu token melalui \texttt{check} dan variasi \texttt{check(..., offset)}. Contohnya, pada \texttt{parseStatement} parser membedakan assignment dan procedure/function call hanya dari token \texttt{ident} yang diikuti \texttt{becomes}, \texttt{lbrack}, atau \texttt{period}. Pola ini meminimalkan backtracking dan tetap kompatibel dengan grammar yang bersifat top-down.
...
Parser mendukung akses array dan record melalui \texttt{parseVariableWithBase} yang membungkus variabel dasar ke dalam \texttt{component-variable} secara rekursif. Setiap komponen dapat berupa index \texttt{[ ... ]} atau akses field \texttt{.ident}, sehingga konstruk seperti \texttt{arr[1].field} dapat diuraikan sebagai rantai node yang berurutan.
