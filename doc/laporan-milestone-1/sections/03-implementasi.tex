...
Lexer ini memiliki 13 state utama sesuai implementasi pada \texttt{lexer.h}. State awal adalah \texttt{START}, sedangkan akhir pembacaan ditangani melalui fungsi \texttt{handleEof()} (bukan state terpisah). Daftar state yang digunakan adalah sebagai berikut.
\begin{enumerate}
    ...
    \item \texttt{SAW\_LESS}. Melihat operator \texttt{<} yang mungkin dilanjutkan menjadi \texttt{<>} atau \texttt{<=}.

    \item \texttt{SAW\_GREATER}. Melihat operator \texttt{>} yang mungkin dilanjutkan menjadi \texttt{>=}.

    \item \texttt{SAW\_EQUAL}. Melihat operator \texttt{=} yang valid hanya jika dilanjutkan menjadi \texttt{==}.

    \item \texttt{IN\_COMMENT}. Sedang membaca isi komentar.

    \item \texttt{SAW\_LPAREN}. Telah membaca karakter \texttt{(} untuk membedakan token \texttt{lparent} dan awal komentar \texttt{(*}.

    \item \texttt{SAW\_STAR\_IN\_COMMENT}. Sedang memproses kemunculan \texttt{*} di dalam komentar untuk mendeteksi penutup \texttt{*)}.
\end{enumerate}
