...
\begin{algorithm}[H]
    \caption{Garis Besar Algoritma Lexer (\texttt{input.txt} $\rightarrow$ \texttt{output.txt})}
...
Lexer ini memiliki 15 state dasar, dimulai dari state awal yaitu START, dan state akhir yaitu FOUND_EOF. State-state lainnya merupakan state penengah yang digunakan untuk menandai identifier yang telah ditemukan.
\begin{enumerate}
    \item \texttt{START}. State awal, menunggu karakter baru untuk menentukan jenis token berikutnya.

    \item \texttt{IN\_IDENT}. Sedang membaca sebuah identifier atau keyword (karakter alfanumerik dan underscore).

    \item \texttt{IN\_NUMBER}. Sedang membaca digit-digit untuk konstanta integer.

    \item \texttt{DOT\_AFTER\_NUM}. Baru saja membaca '.', yang bisa merupakan period (delimiter) atau bagian dari bilangan riil.

    \item \texttt{IN\_REAL}. Sedang membaca bagian desimal dari bilangan riil.

    \item \texttt{IN\_STRING}. Sedang membaca karakter di dalam tanda kutip untuk string literal.

    \item \texttt{SAW\_COLON}. State yang melihat operator \texttt{:} yang mungkin dilanjutkan dengan karakter \texttt{=} untuk membentuk operator definisi \texttt{:=}.
    
    \item \texttt{SAW\_LESS}. State yang melihat operator \texttt{<} yang mungkin dilanjutkan dengan karakter \texttt{>} atau \texttt{=} untuk membentuk operator tidak sama dengan seperti \texttt{<>} atau operator lebih kecil dari atau sama dengan \texttt{<=}.
    
    \item \texttt{SAW\_GREATER}. State yang melihat operator \texttt{>} yang mungkin dilanjutkan dengan karakter \texttt{=} untuk membentuk operator lebih besar dari atau sama dengan \texttt{>=}.
    
    \item \texttt{SAW\_EQUAL}. State yang melihat operator \texttt{=} yang mungkin dilanjutkan dengan karakter \texttt{=} untuk membentuk operator multi-karakter \texttt{==}. Jika tidak dilanjutkan, maka akan di-EMIT eror.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{03-implementasi/diagram-transisi.png}
...
\textbf{Fungsi Utilitas}. File ini juga mendeklarasikan tiga fungsi utilitas penting:
\begin{itemize}
...
File \texttt{token.cpp} mengimplementasikan tiga fungsi utilitas yang dideklarasikan di \texttt{token.h}.
