...
\begin{itemize}
    ...
    \item \texttt{SAW\_COLON}, \texttt{SAW\_LESS}, \texttt{SAW\_GREATER}, \texttt{SAW\_EQUAL}. State yang melihat operator karakter tunggal yang mungkin dilanjutkan dengan karakter lain (untuk membentuk operator multi-karakter seperti \texttt{:=}, \texttt{<>}, \texttt{>=}, \texttt{==}).

    \item \texttt{IN\_COMMENT}. Sedang membaca karakter di dalam komentar (baik \texttt{\{ \ldots \}} maupun \texttt{(* \ldots *)}).

    \item \texttt{SAW\_LPAREN}. Melihat '(', yang bisa merupakan left parenthesis atau awal komentar \texttt{(*...*)}.

    \item \texttt{SAW\_STAR\_IN\_COMMENT}. Di dalam komentar dan melihat '*', yang mungkin merupakan akhir komentar \texttt{*)} .
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{handleInComment()}. Akumulasi karakter dalam comment. Komentar gaya \texttt{\textbackslash\{ \textbackslash\}} ditutup dengan \texttt{\textbackslash\}}, sedangkan \texttt{(* \textbackslash))}  ditutup dengan \texttt{*)} (menggunakan state \texttt{SAW\_STAR\_IN\_COMMENT}).
\end{itemize}
