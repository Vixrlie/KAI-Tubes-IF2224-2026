...
void Lexer::handleStart()
{
    ...
    {
        char next = (pos + 1 < static_cast<int>(source.size())) ? source[pos + 1] : '\0';
        char prev = (pos > 0) ? source[pos - 1] : '\0';
        if (std::isdigit(next) && prev != '.')
        {
            buffer = ".";
